...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        ...
        # Adding unique constraint on 'GeoType', fields ['text']
        db.create_unique('twitter_geotype', ['text'])

        # Changing field 'Geo.coordinates'
        db.alter_column('twitter_geo', 'coordinates', self.gf('django.db.models.fields.CharField')(max_length=255, null=True))

        # Adding unique constraint on 'Country', fields ['text']
        db.create_unique('twitter_country', ['text'])

        # Adding unique constraint on 'BoundingBoxType', fields ['text']
        db.create_unique('twitter_boundingboxtype', ['text'])

        # Changing field 'BoundingBox.coordinates'
        db.alter_column('twitter_boundingbox', 'coordinates', self.gf('django.db.models.fields.CharField')(max_length=255, null=True))

        # Adding unique constraint on 'TimeZone', fields ['time_zone']
        db.create_unique('twitter_timezone', ['time_zone'])

        # Adding unique constraint on 'PlaceType', fields ['text']
        db.create_unique('twitter_placetype', ['text'])

        # Adding unique constraint on 'SourceType', fields ['text']
        db.create_unique('twitter_sourcetype', ['text'])

        # Adding unique constraint on 'Hashtag', fields ['text']
        db.create_unique('twitter_hashtag', ['text'])

        # Adding unique constraint on 'Keyword', fields ['keyword']
        db.create_unique('twitter_keyword', ['keyword'])

        # Adding unique constraint on 'Language', fields ['text']
        db.create_unique('twitter_language', ['text'])


    def backwards(self, orm):
        
        # Removing unique constraint on 'Language', fields ['text']
        db.delete_unique('twitter_language', ['text'])

        # Removing unique constraint on 'Keyword', fields ['keyword']
        db.delete_unique('twitter_keyword', ['keyword'])

        # Removing unique constraint on 'Hashtag', fields ['text']
        db.delete_unique('twitter_hashtag', ['text'])

        # Removing unique constraint on 'SourceType', fields ['text']
        db.delete_unique('twitter_sourcetype', ['text'])

        # Removing unique constraint on 'PlaceType', fields ['text']
        db.delete_unique('twitter_placetype', ['text'])

        # Removing unique constraint on 'TimeZone', fields ['time_zone']
        db.delete_unique('twitter_timezone', ['time_zone'])

        # Removing unique constraint on 'BoundingBoxType', fields ['text']
        db.delete_unique('twitter_boundingboxtype', ['text'])

        # Removing unique constraint on 'Country', fields ['text']
        db.delete_unique('twitter_country', ['text'])

        # Removing unique constraint on 'GeoType', fields ['text']
        db.delete_unique('twitter_geotype', ['text'])

        # Changing field 'Geo.coordinates'
        db.alter_column('twitter_geo', 'coordinates', self.gf('django.db.models.fields.TextField')(null=True))
